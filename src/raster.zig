...
pub fn rasterize_triangle(coords: [3]@Vector(3, f32), canvas: *Canvas) void {
    ...
    // Skip if entirely out of canvas bounds
    const upper_x_bound: f32 = @floatFromInt(canvas.width);
    const upper_y_bound: f32 = @floatFromInt(canvas.height);
    if(xmin > upper_x_bound
    or xmax < 0
    or ymin > upper_y_bound
    or ymax < 0) {
	    return;
    }

	// TODO this is happening because of an error in the transforms somewhere
    if(xmax - xmin > 400) {
	    return;
    }
    if(ymax - ymin > 400) {
    ...
    var py: f32 = ymin;
    while(py < ymax and py < upper_y_bound) : (py += 1) {
        if(py < 0) continue;

		if(py >= upper_y_bound) {
	        std.debug.print("Um why?", .{});
		}

        var px: f32 = xmin;
        while(px < xmax and px < upper_x_bound) : (px += 1) {
            if(px < 0) continue;
